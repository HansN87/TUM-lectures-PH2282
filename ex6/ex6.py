...
def get_TS(data, true_ps,  xmin=x_min, xmax=x_max, gaussian_mean=gaussian_mean, gaussian_sigma=gaussian_sigma):
        # minimize nlogl in p_s
        fmin = lambda p_s: nll(data, p_s, gaussian_mean, gaussian_sigma, xmin, xmax)
        result = optimize.minimize(fmin, [true_ps], bounds=[(0.0, 1.0)])
        logl1 = result.fun

        # get nlogl for p_s = ps_true
        logl0 = nll(data, true_ps, gaussian_mean, gaussian_sigma, xmin, xmax)
        return 2. * (logl0 - logl1)	


def build_TS(true_p_s, ndatasets, sample_size, itype=0, xmin=x_min, xmax=x_max, gaussian_mean=gaussian_mean, gaussian_sigma=gaussian_sigma): 
    '''
    itype: defines the hypothesis test to be inverted to a confidence interval
    itype = 0 (two sided)
    itype = 1 (one sided - upper limit)
    itype = 2 (one sided - lower limit)
    '''

    TS = []
    for k in range(ndatasets):
        pseudo_data = BuildDataset(sample_size, p_s = true_p_s, gaussian_mean = gaussian_mean, gaussian_sigma=gaussian_sigma, xmin=x_min, xmax=x_max) 
        TS.append(get_TS(pseudo_data, true_p_s, gaussian_mean = gaussian_mean, gaussian_sigma=gaussian_sigma, xmin=x_min, xmax=x_max))

    return np.asarray(TS)


def get_approx_pvalue(TS, obs, eps=0.02):
    idx = np.where(TS >= obs)[0]
    p0 = len(TS[idx])*1.0 / len(TS)
    idx = np.where(TS >= obs+eps)[0]
    p1 = len(TS[idx])*1.0 / len(TS)
    idx = np.where(TS >= obs-eps)[0]
    p2 = len(TS[idx])*1.0 / len(TS)
    return np.amax([p0, p1, p2])
